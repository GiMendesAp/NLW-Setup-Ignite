...
export function Habit() {
  ...
  async function fetchHabits() {
    try {
      ...
      console.log(error);
      Alert.alert('Aviso', 'Não foi possível carregar as informações dos hábitos');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleToggleHabit(habitId: string) {
    try {
      ...
      console.log(error)
      Alert.alert('Aviso', 'Não foi possível atualizar o status do hábito')
    }
  }
...

        {
          isDateInPast && (
            <Text className="text-white mt-10 text-center">
              Você não pode editar hábitos de uma data retroativa
            </Text>
          )
        }
      </ScrollView>
